"""Simple config — RAG baseline without HyDE and without CRAG retry.

Pipeline (4 stages, ~2 LLM calls per query):
    rewrite → retrieve (dense + BM25, RRF, reranker) → collect candidates → CoT select
...
BASE_DIR = Path("/home/nailf/ag-rag")
CORPUS_PATH = BASE_DIR / "protocols_corpus.jsonl"
TEST_ZIP = BASE_DIR / "test_set.zip"
TEST_DIR = BASE_DIR / "test_set"
CACHE_DIR = BASE_DIR / "cache"
CACHE_DIR.mkdir(exist_ok=True, parents=True)
EVAL_OUT = BASE_DIR / "eval_results_simple.jsonl"
SUMMARY_OUT = BASE_DIR / "eval_summary_simple.json"
...
GEMMA_URL = "http://92.46.110.71:8887/v1"
GEMMA_MODEL = "/models/gemma-4-26B-A4B-it"

if not TEST_DIR.exists() or len(list(TEST_DIR.glob("*.json"))) == 0:
...
def build_reranker():
    ...
    return CrossEncoder("BAAI/bge-reranker-v2-m3", device="cuda", max_length=1024)
...
def retrieve(search_query, *, dense_model, chunk_embs, chunks, bm25, reranker_model, protocols):
    """Hybrid BGE-M3(query) + BM25(query) → RRF → rerank. No HyDE."""
    ...
    fused = rrf_fuse([dense_q, sparse_hits], top_k=20)
...
def build_graph(deps):
    ...
    def s_retrieve(state):
        top5 = retrieve(state["search_query"],
    ...
    b.add_edge("retrieve", "candidates")
...
def run_one(graph, protocols, test_file):
    ...
    try:
        ...
        return {"test_file": test_file.name, "error": f"load: {e!r}", "correct": False,
                ...
                "candidates": [], "duration_s": 0.0, "query_snippet": ""}
    ...
    if not q or not isinstance(q, str):
        return {"test_file": test_file.name, "error": "empty query", "correct": False,
                ...
                "top5_pids": [], "candidates": [], "duration_s": 0.0, "query_snippet": ""}
    pred, top5, cands, err = None, [], [], None
    try:
